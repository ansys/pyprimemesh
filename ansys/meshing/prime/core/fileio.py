...
from ansys.meshing.prime.autogen.fileiostructs import (
    FileReadParams, FileReadResults, ImportCadParams, ImportCadResults,
    ImportFluentMeshingMeshParams, ImportFluentMeshingMeshResults,
    ImportFluentCaseParams, ImportFluentCaseResults,
    ImportMapdlCdbParams, ImportMapdlCdbResults)
...
class FileIO(_FileIO):
    ...
    def import_cad(self, file_name : str, params: ImportCadParams) -> ImportCadResults:
        ...
        if not params.append:
            res = self._comm.serve(self._model,
                                   "PrimeMesh::Model/Clear",
                                   self._model._object_id,
                                   args={})
            if (res):
                self._model._sync_up_model()

        #print_logs = self._model.python_logger.isEnabledFor(logging.DEBUG)
        args = {"params" : params._jsonify()}
        result = self._comm.import_cad(self._model, file_name, args)
        import_result = ImportCadResults(model = self._model, json_data = result)
        if (import_result.error_code == ErrorCode.NOERROR):
            self._model._sync_up_model()
        return import_result

    def import_fluent_meshing_meshes(self,
                                     file_names : List[str],
        ...
        """
        result = _FileIO.import_fluent_meshing_meshes(self,
                                                      file_names,
                                                      import_fluent_meshing_mesh_params)
        if (result.error_code == ErrorCode.NOERROR):
            self._model._sync_up_model()
    ...
    def import_fluent_case(self, file_name : str,
        ...
        """
        result = _FileIO.import_fluent_case(self, file_name,
                                                      import_fluent_case_params)
        if (result.error_code == ErrorCode.NOERROR):
            self._model._sync_up_model()
        return result

    def import_mapdl_cdb(self, file_name : str,
                         params : ImportMapdlCdbParams) -> ImportMapdlCdbResults:
