...
def _get_numpy_dtype(current_dtype):
    if np.issubdtype(current_dtype, np.int8):
        ...
        raise ValueError(f'Unsupported dtype {current_dtype}for '
                         f'numpy array optimizaation')
...
class JSONEncoder:
    ...
    def iterencode(self, obj):
        ...
        _iterencode = _make_iterencode(
            markers, self.default, _encoder, floatstr,
            self.key_separator, self.item_separator, self.sort_keys,
            self.skipkeys, intstr, vectorstr)
        return _iterencode(obj)

def _make_iterencode(markers, _default, _encoder, _floatstr,
