...
class Part(CoreObject):
    ...
    * TopoVolume: Topoentity representing volumes.
    ...
    def merge_into(self, part_ids : Iterable[int]):
        """ Merge the part with the parts associated with given ids.


        Parameters
        ----------
        part_ids : Iterable[int]
            Part ids to be merged.

        Examples
        --------
        >>> part.merge_into(part_ids)

        """
        args = {"part_ids" : part_ids}
        command_name = "PrimeMesh::Part/MergeInto"
        self._model._print_logs_before_command("merge_into", args)
        self._comm.serve(self._model, command_name, self._object_id, args=args)
        self._model._print_logs_after_command("merge_into")
