...
class FileIO(CoreObject):
    ...
    def write_pmdat(self, file_name : str, file_write_params : FileWriteParams) -> FileWriteResults:
        ...
        Examples
        --------
        >>> results = file_io.export_fluent_meshing_mesh("/tmp/prime_mesh_data.pmdat", prime.FileWriteParams(model = model))
    ...
    def export_fluent_meshing_size_field(self, file_name : str) -> FileWriteResults:
        """ Export active sizefield to Fluent-Meshing's sizefield file (.sf).
        ...
        --------
        >>> file_io = prime.FileIO(model = model)
        >>> results = file_io.export_fluent_meshing_size_field("/tmp/my_sizefield.sf")

        """
        args = {"file_name" : file_name}
        command_name = "PrimeMesh::FileIO/ExportFluentMeshingSizeField"
        self._model._print_logs_before_command("export_fluent_meshing_size_field", args)
        result = self._comm.serve(self._model, command_name, self._object_id, args=args)
        self._model._print_logs_after_command("export_fluent_meshing_size_field", FileWriteResults(model = self._model, json_data = result))
        return FileWriteResults(model = self._model, json_data = result)
