...
class Scaffolder(CoreObject):
    ...
    def get_overlapping_removable_topo_faces(self, topo_faces : Iterable[int], overlap_tol : float) -> Iterable[int]:
        """ Get the overlapping removable topofaces (shadowed by other faces) within the tolerance of input topofaces.
        ...
        topo_faces : Iterable[int]
            Ids of topology faces.
        overlap_tol : float
            Tolerance used identify the overlapping of topology faces for beams.

        Returns
        -------
        Iterable[int]
            Returns list of overlapping topofaces.


        Examples
        --------
        >>> results = scaffolder.get_overlapping_removable_topo_faces([1,2,7,8], 0.001)

        """
        args = {"topo_faces" : topo_faces,
        "overlap_tol" : overlap_tol}
        command_name = "PrimeMesh::Scaffolder/GetOverlappingRemovableTopoFaces"
        self._model._print_logs_before_command("get_overlapping_removable_topo_faces", args)
        result = self._comm.serve(self._model, command_name, self._object_id, args=args)
        self._model._print_logs_after_command("get_overlapping_removable_topo_faces")
        return result
