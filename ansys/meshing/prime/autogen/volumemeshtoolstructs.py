...
class AutoNodeMoveParams(CoreObject):
    ...
    def set_default(
            ...
            restrict_boundary_nodes_along_surface: bool = None,
            n_attempts: int = None):
        """Sets the default values of AutoNodeMoveParams.
    ...
    @staticmethod
    def print_default():
        """Prints the default values of AutoNodeMoveParams.
...
class CheckGridParams(CoreObject):
    """Parameters used to check grid.
    """
    _default_params = {}
    ...
    def __init__(
            ...
            json_data : dict = None,
             **kwargs):
        """Initializes the CheckGridParams.

        Parameters
        ----------
        model: Model
            Model to create a CheckGridParams object with default parameters.
        json_data: dict, optional
            JSON dictionary to create a CheckGridParams object with provided parameters.

        Examples
        --------
        >>> check_grid_params = prime.CheckGridParams(model = model)
        """
        if json_data:
            self.__initialize()
        else:
            ...
            if all_field_specified:
                ...
                if model is None:
                    ...
                    json_data = model._communicator.initialize_params(model, "CheckGridParams")["CheckGridParams"]
                    self.__initialize()
        self._custom_params = kwargs
        if model is not None:
            [ model._logger.warning(f'Unsupported argument : {key}') for key in kwargs ]
    ...
    @staticmethod
    def set_default():
        """Sets the default values of CheckGridParams.

        """
        args = locals()
        [CheckGridParams._default_params.update({ key: value }) for key, value in args.items() if value is not None]

    @staticmethod
    def print_default():
        """Prints the default values of CheckGridParams.

        Examples
        --------
        >>> CheckGridParams.print_default()
        """
        message = ""
        message += ''.join(str(key) + ' : ' + str(value) + '\n' for key, value in CheckGridParams._default_params.items())
        print(message)
...
class VolumeMeshToolResults(CoreObject):
    ...
    def set_default(
            error_code: ErrorCode = None):
        """Sets the default values of VolumeMeshToolResults.
    ...
    @staticmethod
    def print_default():
        """Prints the default values of VolumeMeshToolResults.
...
class CheckGridResults(CoreObject):
    """Result associated with the check grid operation.
    """
    _default_params = {}
    ...
    def __init__(
            ...
            json_data : dict = None,
             **kwargs):
        """Initializes the CheckGridResults.

        Parameters
        ----------
        model: Model
            Model to create a CheckGridResults object with default parameters.
        ...
        json_data: dict, optional
            JSON dictionary to create a CheckGridResults object with provided parameters.

        Examples
        --------
        >>> check_grid_results = prime.CheckGridResults(model = model)
        """
        if json_data:
            self.__initialize(
            ...
            if all_field_specified:
                ...
                if model is None:
                    ...
                    json_data = model._communicator.initialize_params(model, "CheckGridResults")["CheckGridResults"]
                    self.__initialize(
                        has_non_positive_volumes if has_non_positive_volumes is not None else ( CheckGridResults._default_params["has_non_positive_volumes"] if "has_non_positive_volumes" in CheckGridResults._default_params else json_data["hasNonPositiveVolumes"]),
                        has_non_positive_areas if has_non_positive_areas is not None else ( CheckGridResults._default_params["has_non_positive_areas"] if "has_non_positive_areas" in CheckGridResults._default_params else json_data["hasNonPositiveAreas"]),
                        has_invalid_shape if has_invalid_shape is not None else ( CheckGridResults._default_params["has_invalid_shape"] if "has_invalid_shape" in CheckGridResults._default_params else json_data["hasInvalidShape"]),
                        has_left_handed_faces if has_left_handed_faces is not None else ( CheckGridResults._default_params["has_left_handed_faces"] if "has_left_handed_faces" in CheckGridResults._default_params else json_data["hasLeftHandedFaces"]),
                        error_code if error_code is not None else ( CheckGridResults._default_params["error_code"] if "error_code" in CheckGridResults._default_params else ErrorCode(json_data["error_code"])),
                        warning_codes if warning_codes is not None else ( CheckGridResults._default_params["warning_codes"] if "warning_codes" in CheckGridResults._default_params else [WarningCode(data) for data in json_data["warningCodes"]]))
        self._custom_params = kwargs
        if model is not None:
            [ model._logger.warning(f'Unsupported argument : {key}') for key in kwargs ]
        [setattr(type(self), key, property(lambda self, key = key:  self._custom_params[key] if key in self._custom_params else None,
    ...
    def set_default(
            ...
            error_code: ErrorCode = None,
            warning_codes: List[WarningCode] = None):
        """Sets the default values of CheckGridResults.
        ...
        """
        args = locals()
        [CheckGridResults._default_params.update({ key: value }) for key, value in args.items() if value is not None]

    @staticmethod
    def print_default():
        """Prints the default values of CheckGridResults.

        Examples
        --------
        >>> CheckGridResults.print_default()
        """
        message = ""
        message += ''.join(str(key) + ' : ' + str(value) + '\n' for key, value in CheckGridResults._default_params.items())
        print(message)
