...
class LeakPreventionParams(CoreObject):
    ...
    def set_default(
            ...
            max_hole_size: float = None,
            n_expansion_layers: int = None):
        """Sets the default values of LeakPreventionParams.
    ...
    @staticmethod
    def print_default():
        """Prints the default values of LeakPreventionParams.
...
class SetLeakPreventionsResults(CoreObject):
    ...
    def set_default(
            error_code: ErrorCode = None,
            ids: Iterable[int] = None):
        """Sets the default values of SetLeakPreventionsResults.
    ...
    @staticmethod
    def print_default():
        """Prints the default values of SetLeakPreventionsResults.
...
class ContactPreventionParams(CoreObject):
    ...
    def set_default(
            ...
            target_scope: ScopeDefinition = None,
            size: float = None):
        """Sets the default values of ContactPreventionParams.
    ...
    @staticmethod
    def print_default():
        """Prints the default values of ContactPreventionParams.
...
class SetContactPreventionsResults(CoreObject):
    ...
    def set_default(
            error_code: ErrorCode = None,
            ids: Iterable[int] = None):
        """Sets the default values of SetContactPreventionsResults.
    ...
    @staticmethod
    def print_default():
        """Prints the default values of SetContactPreventionsResults.
...
class FeatureRecoveryParams(CoreObject):
    ...
    def set_default(
            ...
            enable_feature_octree_refinement: bool = None,
            size_at_features: float = None):
        """Sets the default values of FeatureRecoveryParams.
    ...
    @staticmethod
    def print_default():
        """Prints the default values of FeatureRecoveryParams.
...
class SetFeatureRecoveriesResults(CoreObject):
    ...
    def set_default(
            ids: Iterable[int] = None,
            error_code: ErrorCode = None):
        """Sets the default values of SetFeatureRecoveriesResults.
    ...
    @staticmethod
    def print_default():
        """Prints the default values of SetFeatureRecoveriesResults.
...
class CheckPrismControlResults(CoreObject):
    """Results associated with check prism controls.
    """
    _default_params = {}
    ...
    def __initialize(
            ...
            error_code: ErrorCode,
            warning_code: WarningCode,
            incorrect_face_zonelet_ids: Iterable[int],
            incorrect_volume_ids: Iterable[int],
            incorrect_control_ids: Iterable[int]):
        self._success = success
        self._error_code = ErrorCode(error_code)
        self._warning_code = WarningCode(warning_code)
        self._incorrect_face_zonelet_ids = incorrect_face_zonelet_ids if isinstance(incorrect_face_zonelet_ids, np.ndarray) else np.array(incorrect_face_zonelet_ids, dtype=np.int32)
        self._incorrect_volume_ids = incorrect_volume_ids if isinstance(incorrect_volume_ids, np.ndarray) else np.array(incorrect_volume_ids, dtype=np.int32)
        self._incorrect_control_ids = incorrect_control_ids if isinstance(incorrect_control_ids, np.ndarray) else np.array(incorrect_control_ids, dtype=np.int32)
    ...
    def __init__(
            ...
            error_code: ErrorCode = None,
            warning_code: WarningCode = None,
            incorrect_face_zonelet_ids: Iterable[int] = None,
            incorrect_volume_ids: Iterable[int] = None,
            incorrect_control_ids: Iterable[int] = None,
            json_data : dict = None,
             **kwargs):
        """Initializes the CheckPrismControlResults.

        Parameters
        ----------
        model: Model
            Model to create a CheckPrismControlResults object with default parameters.
        success: bool, optional
            Tells whether check prism controls was successful.
        error_code: ErrorCode, optional
            Error code associated with check prism control.
        warning_code: WarningCode, optional
            Warning code associated with check prism control.
        incorrect_face_zonelet_ids: Iterable[int], optional
            List of incorrect face zonelets ids.
        incorrect_volume_ids: Iterable[int], optional
            List of incorrect volume ids.
        incorrect_control_ids: Iterable[int], optional
            List of incorrect control ids.
        json_data: dict, optional
            JSON dictionary to create a CheckPrismControlResults object with provided parameters.

        Examples
        --------
        >>> check_prism_control_results = prime.CheckPrismControlResults(model = model)
        """
        if json_data:
            self.__initialize(
                json_data["success"],
                ErrorCode(json_data["errorCode"]),
                WarningCode(json_data["warningCode"]),
                json_data["incorrectFaceZoneletIds"],
                json_data["incorrectVolumeIds"],
                json_data["incorrectControlIds"])
        else:
            all_field_specified = all(arg is not None for arg in [success, error_code, warning_code, incorrect_face_zonelet_ids, incorrect_volume_ids, incorrect_control_ids])
            if all_field_specified:
                self.__initialize(
                    success,
                    error_code,
                    warning_code,
                    incorrect_face_zonelet_ids,
                    incorrect_volume_ids,
                    incorrect_control_ids)
            else:
                if model is None:
                    raise ValueError("Invalid assignment. Either pass model or specify all properties")
                else:
                    json_data = model._communicator.initialize_params(model, "CheckPrismControlResults")["CheckPrismControlResults"]
                    self.__initialize(
                        success if success is not None else ( CheckPrismControlResults._default_params["success"] if "success" in CheckPrismControlResults._default_params else json_data["success"]),
                        error_code if error_code is not None else ( CheckPrismControlResults._default_params["error_code"] if "error_code" in CheckPrismControlResults._default_params else ErrorCode(json_data["errorCode"])),
                        warning_code if warning_code is not None else ( CheckPrismControlResults._default_params["warning_code"] if "warning_code" in CheckPrismControlResults._default_params else WarningCode(json_data["warningCode"])),
                        incorrect_face_zonelet_ids if incorrect_face_zonelet_ids is not None else ( CheckPrismControlResults._default_params["incorrect_face_zonelet_ids"] if "incorrect_face_zonelet_ids" in CheckPrismControlResults._default_params else json_data["incorrectFaceZoneletIds"]),
                        incorrect_volume_ids if incorrect_volume_ids is not None else ( CheckPrismControlResults._default_params["incorrect_volume_ids"] if "incorrect_volume_ids" in CheckPrismControlResults._default_params else json_data["incorrectVolumeIds"]),
                        incorrect_control_ids if incorrect_control_ids is not None else ( CheckPrismControlResults._default_params["incorrect_control_ids"] if "incorrect_control_ids" in CheckPrismControlResults._default_params else json_data["incorrectControlIds"]))
        self._custom_params = kwargs
        if model is not None:
            [ model._logger.warning(f'Unsupported argument : {key}') for key in kwargs ]
        [setattr(type(self), key, property(lambda self, key = key:  self._custom_params[key] if key in self._custom_params else None,
    ...
    def set_default(
            ...
            incorrect_volume_ids: Iterable[int] = None,
            incorrect_control_ids: Iterable[int] = None):
        """Sets the default values of CheckPrismControlResults.
        ...
        """
        args = locals()
        [CheckPrismControlResults._default_params.update({ key: value }) for key, value in args.items() if value is not None]

    @staticmethod
    def print_default():
        """Prints the default values of CheckPrismControlResults.

        Examples
        --------
        >>> CheckPrismControlResults.print_default()
        """
        message = ""
        message += ''.join(str(key) + ' : ' + str(value) + '\n' for key, value in CheckPrismControlResults._default_params.items())
        print(message)
    ...
    def _jsonify(self) -> Dict[str, Any]:
        ...
        json_data["errorCode"] = self._error_code
        json_data["warningCode"] = self._warning_code
        json_data["incorrectFaceZoneletIds"] = self._incorrect_face_zonelet_ids
        json_data["incorrectVolumeIds"] = self._incorrect_volume_ids
        json_data["incorrectControlIds"] = self._incorrect_control_ids
        [ json_data.update({ utils.to_camel_case(key) : value }) for key, value in self._custom_params.items()]
        return json_data

    def __str__(self) -> str:
        message = "success :  %s\nerror_code :  %s\nwarning_code :  %s\nincorrect_face_zonelet_ids :  %s\nincorrect_volume_ids :  %s\nincorrect_control_ids :  %s" % (self._success, self._error_code, self._warning_code, self._incorrect_face_zonelet_ids, self._incorrect_volume_ids, self._incorrect_control_ids)
        message += ''.join('\n' + str(key) + ' : ' + str(value) for key, value in self._custom_params.items())
        return message
    ...
    @property
    def incorrect_face_zonelet_ids(self) -> Iterable[int]:
        """List of incorrect face zonelets ids.
        """
        return self._incorrect_face_zonelet_ids

    @incorrect_face_zonelet_ids.setter
    def incorrect_face_zonelet_ids(self, value: Iterable[int]):
        self._incorrect_face_zonelet_ids = value

    @property
    def incorrect_volume_ids(self) -> Iterable[int]:
        """List of incorrect volume ids.
        """
        return self._incorrect_volume_ids

    @incorrect_volume_ids.setter
    def incorrect_volume_ids(self, value: Iterable[int]):
        self._incorrect_volume_ids = value

    @property
    def incorrect_control_ids(self) -> Iterable[int]:
        """List of incorrect control ids.
        """
        return self._incorrect_control_ids

    @incorrect_control_ids.setter
    def incorrect_control_ids(self, value: Iterable[int]):
        self._incorrect_control_ids = value

class CheckPrismControlParams(CoreObject):
    """Parameters to check prism control.
    """
    _default_params = {}
    ...
    def __init__(
            ...
            json_data : dict = None,
             **kwargs):
        """Initializes the CheckPrismControlParams.

        Parameters
        ----------
        model: Model
            Model to create a CheckPrismControlParams object with default parameters.
        json_data: dict, optional
            JSON dictionary to create a CheckPrismControlParams object with provided parameters.

        Examples
        --------
        >>> check_prism_control_params = prime.CheckPrismControlParams(model = model)
        """
        if json_data:
            self.__initialize()
        else:
            ...
            if all_field_specified:
                ...
                if model is None:
                    ...
                    json_data = model._communicator.initialize_params(model, "CheckPrismControlParams")["CheckPrismControlParams"]
                    self.__initialize()
        self._custom_params = kwargs
        if model is not None:
            [ model._logger.warning(f'Unsupported argument : {key}') for key in kwargs ]
    ...
    @staticmethod
    def set_default():
        """Sets the default values of CheckPrismControlParams.

        """
        args = locals()
        [CheckPrismControlParams._default_params.update({ key: value }) for key, value in args.items() if value is not None]

    @staticmethod
    def print_default():
        """Prints the default values of CheckPrismControlParams.

        Examples
        --------
        >>> CheckPrismControlParams.print_default()
        """
        message = ""
        message += ''.join(str(key) + ' : ' + str(value) + '\n' for key, value in CheckPrismControlParams._default_params.items())
        print(message)
