...
class Graphics(object):
    ...
    def __get_ctrl_point_display_mesh_object(self, part_id, spline_id):
        ...
        id = spline.id
        disp_mesh = _DisplayMesh(display_mesh_type, id, part_id,
            self, self._model, nodes, face_list, False, zone_id=-1,
            zone_name="", part_name=part.name)
        return disp_mesh
    ...
    def __get_face_display_mesh_object(self, part_id : int,
                                      face_facet_res : prime.FaceConnectivityResults,
                                      index : int):
        """      """
        part = self._model.get_part(part_id)
        ...
        disp_mesh = _DisplayMesh(display_mesh_type, id, part_id, self, self._model,
            ...
            has_mesh, zone_id=face_facet_res.face_zone_ids[index],
            zone_name=face_facet_res.face_zone_names[index], part_name=part.name)
        return disp_mesh

    def __get_edge_display_mesh_object(self, part_id : int,
        ...
        disp_mesh = _DisplayMesh(display_mesh_type, id, part_id, self, self._model,
            ...
            has_mesh, part_name=part.name, topo_edge_type=edge_facet_res.topo_edge_types[index],
            number_of_edges=edge_facet_res.num_edges_per_edge_zonelet[index])
        return disp_mesh

    def __call__(self, parts = None,
                 update = True, spline = False,
    ...
    def __color_by_type_callback(self, flag):
        ...
        if(self._color_by_type == ColorByType.ZONELET):
            color_by_type_icon_file = os.path.join(os.path.dirname(__file__),
                                      'images', 'surface_body.png')
        ...
        [disp_mesh.set_color_by_type(self._color_by_type)
            ...
            if data.get("faces") != None for disp_mesh in data["faces"]]
    ...
    def get_face_mesh_data(self):
        """     """
        face_mesh_data = [disp_mesh for part_id,
                          data in self._display_data.items()
    ...
    def show(self, update = False):
        ...
        self._colorByTypeBt = self._plotter.add_checkbox_button_widget(
                                            ...
                                            position=(10, 700), size = 30, border_size=3)
        self._hideBt = self._plotter.add_checkbox_button_widget(
                                        self.__hide_unhide_selection,
                                        position=(10, 650), size = 30,
                                        border_size=3)
        self._showEdgeBt = self._plotter.add_checkbox_button_widget(
                                            self.__show_edges_callback,
                                            value = False, position=(10, 600),
                                            size = 30, border_size=3)
        ...
        self._printInfoBt = self._plotter.add_checkbox_button_widget(
                                             ...
                                             position=(10, 550), size = 30, border_size=3)
    ...
    def __draw_parts(self, parts= [], update = False, spline = False):
        ...
        if spline:
            [disp_mesh.add_to_plotter(self._plotter)
            for part_id, data in self._display_data.items()
            if (part_id in parts) for key, disp_mesh_data in data.items()
            for disp_mesh in disp_mesh_data]
        else:
            [disp_mesh.add_to_plotter(self._plotter)
            for part_id, data in self._display_data.items()
            if (part_id in parts) for key, disp_mesh_data in data.items()
            for disp_mesh in disp_mesh_data
            if(disp_mesh.type != DisplayMeshType.SPLINECONTROLPOINTS or
            disp_mesh.type != DisplayMeshType.SPLINESURFACE)]
        ...
        self._hideBt = self._plotter.add_checkbox_button_widget(
                                        self.__hide_unhide_selection,
                                        position=(10, 650), size = 30, border_size=3)
        self._showEdgeBt = self._plotter.add_checkbox_button_widget(
                                            self.__show_edges_callback,
                                            value = False, position=(10, 600),
                                            size = 30, border_size=3)
    ...
    def __draw_scope_parts(self, update = False, scope : prime.ScopeDefinition = None):
        ...
        for part_id in parts:
            ...
            if(disp_data is not None):
                [disp_mesh.add_to_plotter(self._plotter)
                for disp_mesh in disp_data if (disp_mesh._id in disp_ids)]
    ...
    def __update_bt_icons(self):
        ...
        r = _vtk.vtkPNGReader()
        color_by_zone_icon_file = os.path.join(os.path.dirname(__file__),
                                    'images', 'bin.png')
        ...
        hide_unhide_icon_file = os.path.join(os.path.dirname(__file__),
                                    'images', 'invert_visibility.png')
        ...
        show_edges_icon_file = os.path.join(os.path.dirname(__file__),
                                   'images', 'show_edges.png')
        ...
        print_info_icon_file = os.path.join(os.path.dirname(__file__),
                                   'images', 'selectioninfo.png')
...
class _DisplayMesh(object):
    """     """
    def __init__(self, type : DisplayMeshType, id : int, part_id : int,
                 graphics : Graphics, model : prime.Model, vertices : np.array,
                 facet_list : np.array, has_mesh : bool, zone_id  : int = 0,
                 zone_name : str = "", part_name : str = "", topo_edge_type : int = 0,
    ...
    def add_to_plotter(self, plotter : Plotter):
        ...
        if(self._type == DisplayMeshType.TOPOFACE or self._type == DisplayMeshType.FACEZONELET):
            ...
            if self._poly_data.n_points > 0:
                self._actor = plotter.add_mesh(self._poly_data,
                                     show_edges=sh_edge, scalars="colors",
                                     rgb=True, pickable=True)
        elif(self._type == DisplayMeshType.TOPOEDGE or self._type == DisplayMeshType.EDGEZONELET):
            if(self._poly_data == None):
                edge = pv.PolyData()
            ...
            if self._poly_data.n_points > 0:
                self._actor = plotter.add_mesh(self._poly_data,
                                     scalars="colors", rgb=True, line_width=4)
            ...
            if self._poly_data.n_points > 0:
                self._actor = plotter.add_mesh(self._poly_data,
                                    show_edges=True, scalars="colors",
                                    rgb=True, pickable=False,
                                    style='wireframe', edge_color=[0, 0, 255])
            ...
            if self._poly_data.n_points > 0:
                self._actor = plotter.add_mesh(self._poly_data,
                                     show_edges=False, scalars="colors",
                                     rgb=True, pickable=False)
    ...
    def unhide(self, plotter : Plotter):
        if(self._actor is None and self._poly_data.n_points > 0):
            self._actor = plotter.add_mesh(self._poly_data,
                                    ...
                                    scalars="colors", rgb=True, pickable=True)
