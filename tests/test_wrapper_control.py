...
def test_wrapper_control(get_remote_client):
    ...
    model.set_global_sizing_params(prime.GlobalSizingParams(model = model, min = 2.0, max = 20, growth_rate=1.2))
    ...
    con_prev_params1 = prime.ContactPreventionParams(model = model, source_scope=sscope, target_scope=tscope, size=0.2)
    ...
    con_prev_params2 = prime.ContactPreventionParams(model = model, source_scope=sscope2, target_scope=tscope2, size=0.1)
    ...
    leak_prev_params1 = prime.LeakPreventionParams(model = model, scope=scope1, max_hole_size=0.2, material_points=["cabin"])
    scope2 = prime.ScopeDefinition(model = model, part_expression="breaks2*, *tyres2*")
    leak_prev_params2 = prime.LeakPreventionParams(model = model, scope=scope2, max_hole_size = 0.1, material_points=["cargo"])
    ...
    feat_params1 = prime.FeatureRecoveryParams(model = model, scope=scope, enable_feature_octree_refinement=True, size_at_features=0.3)
    ...
    wrapper_control = prime.local_model().control_data.create_wrapper_control()
    wrapper_control.set_geometry_scope(prime.ScopeDefinition(model=model, entity_type=prime.ScopeEntity.FACEANDEDGEZONELETS))
