...
def test_case_0(get_remote_client):
    ...
    case_0 = prime.examples.download_file("case-0.pmdat","https://github.com/pyansys/example-data/tree/master/pyprimemesh/case_0",destination="E:\Github\AnsysDev\Dev\Github\pyprimemesh\examples")
    # reads file
    ...
    model.set_global_sizing_params(params=prime.GlobalSizingParams(model = model, min = 0.2, max = 10.0, growth_rate = 1.2))
    ...
    res = size_field.compute_volumetric(ids, prime.VolumetricSizeFieldComputeParams(enable_multi_threading=False))
    #Surface mesh
    ...
    for part in model.parts:
        ...
        params = prime.SurferParams(model = model, size_field_type= prime.SizeFieldType.VOLUMETRIC)
        result = surfer.mesh_topo_faces(part.id, topo_faces = part.get_topo_faces(),params = params)
    ...
    res_geom_all = upperpart.delete_topo_entities(params = prime.DeleteTopoEntitiesParams(model=model, delete_geom_zonelets=True, delete_mesh_zonelets=False))
    res_geom_all = middlepart.delete_topo_entities(params = prime.DeleteTopoEntitiesParams(model=model, delete_geom_zonelets=True, delete_mesh_zonelets=False))
    res_geom_all = lowerpart.delete_topo_entities(params = prime.DeleteTopoEntitiesParams(model=model, delete_geom_zonelets=True, delete_mesh_zonelets=False))
    res_geom_all = cylpart.delete_topo_entities(params = prime.DeleteTopoEntitiesParams(model=model, delete_geom_zonelets=True, delete_mesh_zonelets=False))
    ...
    subParam = prime.SubtractZoneletsParams(model = model, retain_cutter = False, extract_edges = False, trace_edges = False)
    zonelets = part.get_face_zonelets_of_label_name_pattern("middle", prime.NamePatternParams(model))
    print(zonelets)
    cutters = []
