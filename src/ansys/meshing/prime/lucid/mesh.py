...
class Mesh:
    ...
    * Helper methods for reading and writing files.
    ...
    def subtract_mesh_part(
        self, target_part: prime.Part, cutter_parts: List[prime.Part], retain_cutter: bool = False
    ):
        """Subtract many tool parts from a target part.
        Parts are assumed to be closed volumes and cannot contain topology.

        Parameters
        -------------
        target_part : prime.Part
            Target part to be operated on.

        cutter_parts : List[prime.Part]
            Parts to subtract from target part.

        retain_cutter : bool
            Keep or remove cutter tools.

        """
        if len(target_part.get_topo_faces()) > 0:
            prime.PrimeRuntimeError("Operation not supported on parts with topology")
        for part in cutter_parts:
            if len(part.get_topo_faces()) > 0:
                prime.PrimeRuntimeError("Operation not supported on parts with topology")

        tools = [
            prime.PartZonelets(self._model, tool.id, tool.get_face_zonelets())
            for tool in cutter_parts
        ]
        con = prime.SurfaceUtilities(self._model)
        con.subtract_zonelets(
            target_part.id,
            target_part.get_face_zonelets(),
            tools,
            prime.SubtractZoneletsParams(
                model=self._model, retain_cutter=retain_cutter, extract_edges=True, trace_edges=True
            ),
        )
