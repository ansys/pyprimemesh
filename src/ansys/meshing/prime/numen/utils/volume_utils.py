...
def create_cap(model: prime.Model, part: prime.Part, side_face_exp: str,
               side_face_evaluation_type: str, cap_zone_name: str):
    ...
    if side_face_evaluation_type == "labels":
        face_zonelets = part.get_face_zonelets_of_label_name_pattern(
            side_face_exp, name_pattern_params,
        )
    ...
    if zone_id <= 0:
        create_zone_res = model.create_zone(
...
def detect_thin_volumes(model: prime.Model, part: prime.Part, thickness: float,
                        volume_zone_exp: str, source_face_label: str, target_face_label: str):
    vt = PrimeObj(model, "VTFeatureRecognition", part.id)
    command_name = "PrimeMesh::Part/GetTopoVolumesOfZoneNamePattern_Beta"
    args = { "zone_name_pattern" : volume_zone_exp,
...
def volume_zone_management(model: prime.Model, part: prime.Part, fluid_zones: List[str]):
    ...
    for volume_zone in volume_zone_ids:
        ...
        volume_zone_boundary_zonelets = [item for item in volume_zone_face_zonelets
                                         if item not in face_zone_zonelets]
...
def create_thin_volume_controls(model: prime.Model, part: prime.Part,
                                source_face_label_prefix: str,
                                target_face_label_prefix: str,
                                imprint_sides: bool, n_layers: int, stair_step: bool):
    ...
    for src_label in labels:
        if src_label.startswith(source_face_label_prefix):
            ...
            if len(volume_zone_names) > 0:
                source_scope = prime.ScopeDefinition(model = model,
                                            entity_type = prime.ScopeEntity.FACEZONELETS,
                                            evaluation_type = prime.ScopeEvaluationType.LABELS,
                                            part_expression = part.name,
                                            label_expression = src_label)
                target_scope = prime.ScopeDefinition(model = model,
                                            entity_type = prime.ScopeEntity.FACEZONELETS,
                                            evaluation_type = prime.ScopeEvaluationType.LABELS,
                                            part_expression = part.name,
                                            label_expression = trg_label)
                volume_scope = prime.ScopeDefinition(model = model,
                                            entity_type = prime.ScopeEntity.VOLUME,
                                            evaluation_type = prime.ScopeEvaluationType.ZONES,
                                            part_expression = part.name,
                                            zone_expression = ",".join(volume_zone_names))
                thin_volume_params = prime.ThinVolumeMeshParams(model = model,
                                            n_layers = n_layers,
...
def create_prism_control(model: prime.Model, part: prime.Part, face_expression: str,
                          face_evaluation_type: prime.ScopeEvaluationType,
                          volume_expression: str, n_layers: int, first_height: float,
                          last_aspect_ratio: float):
    prism_ctrl = model.control_data.create_prism_control()
    prism_params = prime.PrismControlGrowthParams(model = model, n_layers = n_layers,
