...
class TolerantConnect:
    ...
    *
    ...
    def _collapse_within_parts(
            self,
            collapse_within_parts_name_exp: str = "",
        ...
        if (n_parts > 1):
            ...
            for part_i in range(1, n_parts):
                ...
                merge_res = self._model.merge_parts(
                    parts_to_merge,
                    prime.MergePartsParams(model = self._model,
                ...
                merged_part.remove_labels_from_zonelets([dummy_unique_label1, dummy_unique_label2],
                                                        all_face_zonelets)
    ...
    def delete_topology(
        ...
        for part in self._model.parts:
            if part.get_topo_faces():
                ...
                self._logger.info(
                    "No topology to delete in part: " + part.name
                    )
    ...
    def fuse(
        ...
        for part in self._model.parts:
            ...
            for non_share_label in non_share_labels:
                ...
                if prime.WarningCode.FUSEOVERLAPREMOVALINCOMPLETE in result.warning_codes:
                    self._logger.warning(
                        'Failed to remove overlaps in part'
                        + part.name
                        )
                if prime.WarningCode.REMOVEOVERLAPWITHINTERSECT in result.warning_codes:
                    self._logger.warning(
                        'Intersections found when removing overlaps in part'
                        + part.name
                        )
        vol_part_name = ""
        cont_part_name = ""
        ...
        if len(self._model.parts) > 1:
            ...
            if len(interfering_parts_exp) > 0:
                ...
                self._resolve_conflicts(name = part_name, all_labels_in  = part_labels,
                                    priority_ordered_part_names_in = interfering_parts_priority,
                                    new_vol_zone_name = "unclaimed_vol_zone")
            tolerant_connect_part.remove_labels_from_zonelets(part_labels,
