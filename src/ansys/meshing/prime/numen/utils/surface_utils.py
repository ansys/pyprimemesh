...
def improve_quality(model: prime.Model, part: prime.Part, soft_target_skewness: float,
                     hard_target_skewness: float, local_remesh_by_size_change: bool,
    ...
    thin_strip_params = prime.SearchByThinStripParams(model,
...
def cleanup_tri_mesh(model: prime.Model, part: prime.Part, collapse_sliver_faces: bool,
                     stitch_free_faces: bool, keep_small_free_surfaces: bool, logger: Logger):
    quality_reg_id = 26
    surface_search_tool = prime.SurfaceSearch(model)
    collapse_tool = prime.CollapseTool(model)
    quality_params = prime.SearchByQualityParams(model = model,
                                quality_limit = 0.98,
...
def diahedral_angle_results(model: prime.Model, part: prime.Part):
    ...
    params =  {
        "minAngle" : 5,
        "faceOrientationCorrectionType" : 1
    }
    args = {
        "part_id" : part.id,
        "face_zonelets" : part.get_face_zonelets(),
